...
export class ProjectController {
  ...
  createProject = async (req: Request, res: Response) => {
    ...
    await this.projectMemberRepo.addMember(project.id, createdBy, ProjectUserRole.MANAGER);

    // Helper to add lists
    const addList = async (ids: number[] | undefined, role: ProjectUserRole) => {
      if (!Array.isArray(ids)) return;
      for (const uid of ids) {
        const target = await this.userRepo.findById(uid);
        if (!target) continue; // skip nonexistent users

        const existing = await this.projectMemberRepo.findMember(project.id, uid);
        if (existing) continue;

        await this.projectMemberRepo.addMember(project.id, uid, role);
      }
    };
    ...
    const full = await this.projectRepo.findById(project.id);
  ...
  addMember = async (req: Request, res: Response) => {
    ...
    const targetUser = await this.userRepo.findById(Number(userId));
    if (!targetUser) throw notFound("User not found.");

    const existing = await this.projectMemberRepo.findMember(projectId, Number(userId));
    if (existing) throw conflict("User is already a member of this project.");

    const member = await this.projectMemberRepo.addMember(projectId, Number(userId), role as ProjectUserRole);
